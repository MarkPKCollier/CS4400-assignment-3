...
def test2():
    ...
    # user 2 should start a transaction and make edits to the files
    # user 1 should read the contents of the files and verify they are the same as when created
    # user 2 should cancel the transaction
    # both users should verify that they have the old version of the file
    ...
    r = requests.get(file_server_addr, params=session_mgr_2.encrypt_msg({
            'operation': 'fetch',
            'file_id': file_id_1
        }, 'file server'))
    res = session_mgr_2.decrypt_msg(r.json(), 'file server')
